...
const CreateVisit: React.FC = () => {
  ...
  useEffect(() => {
    if (storeDoctors.length > 0 && storeProducts.length > 0) {
      // Use data from store only
      setDoctors(storeDoctors);
      setProducts(storeProducts);
      setLoadingData(false);
    } else {
      // No data in store, show message to user
      toast({
        title: "تنبيه",
        description: "يرجى زيارة صفحة 'بياناتي' أولاً لتحميل البيانات",
        variant: "destructive",
      });
      setLoadingData(false);
    }
  }, [storeDoctors, storeProducts, toast]);
