  ...
  const createBarChart = (data: any[], containerId: string, title: string) => {
    ...
    g.selectAll(".bar")
      .on("mouseover", function (event, d) {
        ...
        tooltip
          .html(`<div style="text-align: center;"><strong>${d.name}</strong><br/>القيمة: ${d.value}</div>`)
  ...
  const createPieChart = (data: any[], containerId: string, title: string) => {
    ...
    slices.selectAll("path")
      .on("mouseover", function(event, d) {
        ...
        tooltip
          .html(`<div style="text-align: center;"><strong>${d.data.name}</strong><br/>الكمية: ${d.data.value}<br/>النسبة: ${percentage}%</div>`)
          .style("left", event.pageX + 10 + "px")
          .style("top", event.pageY - 40 + "px");
      })
      .on("mouseout", function() {
    ...
    if (data && data.areaAnalytics && data.areaAnalytics.length > 0) {
      ...
      data.areaAnalytics.forEach((area: any) => {
        if (area.doctorVisits.productsDistributed) {
          Object.entries(area.doctorVisits.productsDistributed).forEach(([product, quantity]: [string, any]) => {
            doctorProducts[product] = (doctorProducts[product] || 0) + quantity;
          });
        }
      });
      
      const doctorChartData = Object.entries(doctorProducts).map(([name, value]) => ({ name, value }));
      ...
      data.areaAnalytics.forEach((area: any) => {
        if (area.pharmacyVisits.productsOrdered) {
          Object.entries(area.pharmacyVisits.productsOrdered).forEach(([product, quantity]: [string, any]) => {
            pharmacyProducts[product] = (pharmacyProducts[product] || 0) + quantity;
          });
        }
      });
      
      const pharmacyChartData = Object.entries(pharmacyProducts).map(([name, value]) => ({ name, value }));
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <Select value={filters.detailLevel} onValueChange={(value) => handleFilterChange('detailLevel', value)}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold mb-2">{data.summary.totalSamplesDistributed}</p>
                    <div className="flex items-center text-xs text-orange-200">
                      <span className="bg-orange-400 bg-opacity-30 px-2 py-1 rounded-full">
                        {(data.summary.totalSamplesDistributed / data.summary.totalDoctorVisits).toFixed(1)} لكل زيارة
                      </span>
                    </div>
                  </div>
                  <div className="bg-white bg-opacity-20 rounded-full p-4 backdrop-blur-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {data.areaAnalytics.map((area: any, index: number) => (
                    <div key={index} className="bg-gradient-to-r from-gray-50 to-gray-100 p-4 rounded-lg border">
                      <h4 className="font-semibold text-gray-900 mb-3">{area.areaName} - {area.city}</h4>
                      <div className="space-y-3">
                        <div className="flex justify-between items-center">
                          <span className="text-sm text-gray-600">تكرار زيارات الأطباء</span>
                          <span className="font-semibold text-blue-600">{area.performanceMetrics.doctorVisitFrequency}</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-sm text-gray-600">معدل طلبيات الصيدليات</span>
                          <span className="font-semibold text-green-600">{area.performanceMetrics.pharmacyOrderRate}</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-sm text-gray-600">متوسط قيمة الطلبية</span>
                          <span className="font-semibold text-purple-600">{area.performanceMetrics.averageOrderValue}</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-sm text-gray-600">العينات لكل زيارة</span>
                          <span className="font-semibold text-orange-600">{area.performanceMetrics.samplesPerVisit}</span>
                        </div>
                      </div>
                    </div>
                  ))}
                    ...
                    <div key={index} className="border-b border-gray-200 pb-4 last:border-b-0">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-gray-600">مندوبين طبيين: {area.doctorVisits.medicalReps}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                          <span className="text-gray-600">مندوبين مبيعات: {area.pharmacyVisits.salesReps}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <div className="w-2 h-2 bg-purple-500 rounded-full"></div>
                          <span className="text-gray-600">إجمالي الفريق: {area.combinedStats.totalTeamMembers}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <div className="w-2 h-2 bg-orange-500 rounded-full"></div>
                          <span className="text-gray-600">منتجات فريدة: {area.combinedStats.uniqueProducts}</span>
                        </div>
                      </div>
                    </div>
                  ))}
                    ...
                    <div key={index} className="bg-gradient-to-r from-blue-50 to-indigo-50 p-3 rounded-lg border border-blue-200">
                      ...
                      <div className="space-y-1 text-xs">
                        <div className="flex justify-between">
                          ...
                          <span className="font-semibold text-blue-600">{((area.doctorVisits.totalVisits / area.doctorVisits.medicalReps) * 100 / 30).toFixed(1)}%</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">معدل التحويل:</span>
                          <span className="font-semibold text-green-600">{((area.pharmacyVisits.totalVisits / area.doctorVisits.totalVisits) * 100).toFixed(1)}%</span>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="space-y-3">
                  {data.areaAnalytics.slice(0, 3).map((area: any, index: number) => (
                    <div key={index} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                      ...
                      <span className="text-sm text-gray-600">{area.combinedStats.totalActivities} نشاط</span>
                    </div>
                  ))}
                </div>
              </CardContent>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2 p-2 bg-red-50 rounded border border-red-200">
                        ...
                        <span className="text-sm">انخفاض في منطقة {data.areaAnalytics.length > 0 ? data.areaAnalytics.reduce((min: any, area: any) => (area.doctorVisits?.totalVisits || 0) < (min.doctorVisits?.totalVisits || 0) ? area : min).areaName : 'غير محدد'}</span>
                      </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2 p-2 bg-green-50 rounded border border-green-200">
                        <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                        <span className="text-sm">توسع في منطقة {data.areaAnalytics.length > 0 ? data.areaAnalytics.reduce((max: any, area: any) => (area.doctorVisits?.totalVisits || 0) > (max.doctorVisits?.totalVisits || 0) ? area : max).areaName : 'غير محدد'}</span>
                      </div>
              ...
              <CardContent>
                <div className="space-y-3">
                  {data.topPerformingAreas.slice(0, 5).map((area, index) => (
                    <div key={area.areaName} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                      <div>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-gray-200">
                          <th className="text-right py-3 px-4 font-semibold text-gray-700">اسم المنتج</th>
                          <th className="text-right py-3 px-4 font-semibold text-gray-700">الكمية الموزعة</th>
                          <th className="text-right py-3 px-4 font-semibold text-gray-700">المنطقة</th>
                        </tr>
                      </thead>
                      <tbody>
                        {data.areaAnalytics.map((area: any, areaIndex: number) => 
                          area.doctorVisits.productsDistributed && 
                          Object.entries(area.doctorVisits.productsDistributed).map(([product, quantity]: [string, any], productIndex: number) => (
                            <tr key={`${areaIndex}-${productIndex}`} className="border-b border-gray-100 hover:bg-blue-50 transition-colors">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-gray-200">
                          <th className="text-right py-3 px-4 font-semibold text-gray-700">اسم المنتج</th>
                          <th className="text-right py-3 px-4 font-semibold text-gray-700">الكمية المطلوبة</th>
                          <th className="text-right py-3 px-4 font-semibold text-gray-700">المنطقة</th>
                        </tr>
                      </thead>
                      <tbody>
                        {data.areaAnalytics.map((area: any, areaIndex: number) => 
                          ...
                          Object.entries(area.pharmacyVisits.productsOrdered).map(([product, quantity]: [string, any], productIndex: number) => (
                            <tr key={`${areaIndex}-${productIndex}`} className="border-b border-gray-100 hover:bg-green-50 transition-colors">
                              <td className="py-3 px-4 font-medium text-gray-900">{product}</td>
                              <td className="py-3 px-4">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-200">
                        <th className="text-right py-3 px-4 font-semibold text-gray-700">اسم المنتج</th>
                        <th className="text-right py-3 px-4 font-semibold text-gray-700">العينات الموزعة (أطباء)</th>
                        <th className="text-right py-3 px-4 font-semibold text-gray-700">الطلبيات (صيدليات)</th>
                        <th className="text-right py-3 px-4 font-semibold text-gray-700">معدل التحويل</th>
                      </tr>
                    </thead>
                    <tbody>
                      {(() => {
                        const productComparison: { [key: string]: { doctor: number; pharmacy: number } } = {};
                        
                        // Collect doctor products
                        data.areaAnalytics.forEach((area: any) => {
                          if (area.doctorVisits.productsDistributed) {
                            Object.entries(area.doctorVisits.productsDistributed).forEach(([product, quantity]: [string, any]) => {
                              if (!productComparison[product]) productComparison[product] = { doctor: 0, pharmacy: 0 };
                        ...
                        return Object.entries(productComparison).map(([product, data], index) => {
                          const conversionRate = data.doctor > 0 ? ((data.pharmacy / data.doctor) * 100).toFixed(1) : '0';
                          return (
              ...
              <Card className="xl:col-span-2">
                ...
                <CardContent>
                  <div className="space-y-6">
                    {data.areaAnalytics.map((area: any, index: number) => {
                      const efficiency = ((area.doctorVisits.totalVisits / area.doctorVisits.medicalReps) * 100 / 30);
                      const conversionRate = ((area.pharmacyVisits.totalVisits / area.doctorVisits.totalVisits) * 100);
                      const samplesPerVisit = (area.doctorVisits.products && area.doctorVisits.products.length > 0) 
                        ? area.doctorVisits.products.reduce((sum: number, p: any) => sum + p.samplesDistributed, 0) / area.doctorVisits.totalVisits
                        : 0;
                      
                      return (
                        <div key={index} className="bg-gradient-to-r from-slate-50 to-gray-100 p-4 rounded-xl border border-gray-200 hover:shadow-lg transition-all duration-300">
                          <div className="flex items-center justify-between mb-4">
                            <h4 className="font-bold text-gray-900">{area.areaName} - {area.city}</h4>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div className="text-center">
                              <div className="text-2xl font-bold text-blue-600">{area.doctorVisits.totalVisits}</div>
                              <div className="text-xs text-gray-600">زيارات أطباء</div>
                            </div>
                            <div className="text-center">
                              <div className="text-2xl font-bold text-green-600">{area.pharmacyVisits.totalVisits}</div>
                            ...
                            <div className="text-center">
                              <div className="text-2xl font-bold text-orange-600">{samplesPerVisit.toFixed(1)}</div>
                          ...
                          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div className="bg-white p-3 rounded-lg border">
                              <div className="text-sm font-medium text-gray-700 mb-1">كفاءة الفريق</div>
                              ...
                              <div className="text-xs text-gray-600 mt-1">{efficiency.toFixed(1)}% من الهدف</div>
                            </div>
                            
                            <div className="bg-white p-3 rounded-lg border">
                              <div className="text-sm font-medium text-gray-700 mb-1">تقييم الأداء</div>
                              <div className="flex items-center gap-1">
                                {[1,2,3,4,5].map((star) => (
                                  <svg key={star} className={`w-4 h-4 ${
                                    star <= Math.ceil(efficiency/20) ? 'text-yellow-400' : 'text-gray-300'
                                  }`} fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                                  </svg>
                                ))}
                                <span className="text-xs text-gray-600 ml-2">{Math.ceil(efficiency/20)}/5</span>
                              </div>
                            </div>
                          </div>
                        </div>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="bg-gradient-to-r from-blue-50 to-cyan-50 p-4 rounded-lg border border-blue-200">
                      ...
                      <p className="text-sm text-blue-700">زيادة عدد الزيارات في المناطق ذات الأداء المنخفض بنسبة 25%</p>
                    </div>
                    ...
                    <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-lg border border-green-200">
                      ...
                      <p className="text-sm text-green-700">التركيز على المنتجات عالية الطلب في المناطق النشطة</p>
                    </div>
                    ...
                    <div className="bg-gradient-to-r from-purple-50 to-violet-50 p-4 rounded-lg border border-purple-200">
                      ...
                      <p className="text-sm text-purple-700">تدريب إضافي للمندوبين في المناطق ذات الأداء المنخفض</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
                    ...
                    <div className="bg-gradient-to-r from-amber-50 to-yellow-50 p-4 rounded-lg border border-amber-200">
                      <div className="text-center">
                        <div className="text-3xl font-bold text-amber-600 mb-1">
                          {data.summary.totalDoctorVisits > 0 ? (data.summary.totalSamplesDistributed / data.summary.totalDoctorVisits).toFixed(2) : '0.00'}
                        </div>
                    ...
                    <div className="bg-gradient-to-r from-rose-50 to-pink-50 p-4 rounded-lg border border-rose-200">
                      <div className="text-center">
                        <div className="text-3xl font-bold text-rose-600 mb-1">
                          {data.summary.totalDoctorVisits > 0 ? ((data.summary.totalPharmacyVisits / data.summary.totalDoctorVisits) * 100).toFixed(1) : '0.0'}%
                        </div>
                    ...
                    <div className="bg-gradient-to-r from-teal-50 to-cyan-50 p-4 rounded-lg border border-teal-200">
                      <div className="text-center">
                        <div className="text-3xl font-bold text-teal-600 mb-1">
                          {data.areaAnalytics.reduce((sum: number, area: any) => sum + (area.combinedStats?.totalTeamMembers || 0), 0)}
                        </div>
                    ...
                    <div className="bg-gradient-to-r from-indigo-50 to-blue-50 p-4 rounded-lg border border-indigo-200">
                      <div className="text-center">
                        <div className="text-3xl font-bold text-indigo-600 mb-1">
                          {data.areaAnalytics.reduce((sum: number, area: any) => sum + (area.combinedStats?.uniqueProducts || 0), 0)}
                        </div>
